...
class GoToSafariRouteHandler : RequestHandlerWithContextProtocol {
    func handle<R>(request: R, withContext context: UIViewController, completion: @escaping (R.TResponse) throws -> Void) throws where R : RequestProtocol {
        let request = request as! GoToSafariRoute
        ...
        let safariModel = SafariInputModel(message: message,
                                           ...
                                           givtObj: request.donations.map { SafariTransactionModel(Amount: $0.amount, CollectId: $0.collectId, Timestamp: $0.timeStamp, BeaconId: $0.beaconId)},
    ...
    func canHandle<R>(request: R) -> Bool where R : RequestProtocol {
        return request is GoToSafariRoute
    }
}
