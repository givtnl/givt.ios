...
final class GivtManager: NSObject {
    ...
    func give(antennaID: String, organisationName: String?) {
        ...
        //Do not give in background when we will finish a donation via the credit card flow in Safari
        let country = try! mediater.send(request: GetCountryQuery())
        if country != "US" || UserDefaults.standard.mandateSigned {
