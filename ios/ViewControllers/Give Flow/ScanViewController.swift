...
class ScanViewController: BaseScanViewController {
    ...
    @objc func removeOverlay() {
        ...
        UserDefaults.standard.showCasesByUserID.append(AppConstants.Showcase.giveDifferently.rawValue)
    }
    
    func showGiveDifferentlyShowcase() {
        if UserDefaults.standard.showCasesByUserID.contains(AppConstants.Showcase.giveDifferently.rawValue) {
            return
        }
