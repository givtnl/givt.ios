...
class AmountViewController: UIViewController, UIGestureRecognizerDelegate, NavigationManagerDelegate, MaterialShowcaseDelegate {
    ...
    func showCaseDidDismiss(showcase: MaterialShowcase) {
        if showcase.primaryText == NSLocalizedString("Ballon_ActiveerCollecte", comment: "") {
            if !UserDefaults.standard.showCasesByUserID.contains(AppConstants.Showcase.giveSituation.rawValue) {
                showShowcase(message: NSLocalizedString("GiveSituationShowcaseTitle", comment: "") + " 😉", targetView: btnGive)
                UserDefaults.standard.showCasesByUserID.append(AppConstants.Showcase.giveSituation.rawValue)
            }
        }
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if UserDefaults.standard.viewedCoachMarks >= 2 && !UserDefaults.standard.showCasesByUserID.contains(AppConstants.Showcase.giveSituation.rawValue) {
    ...
    func showFirstBalloon() {
        if UserDefaults.standard.viewedCoachMarks != 0 || UserDefaults.standard.showCasesByUserID.contains(AppConstants.Showcase.multipleCollects.rawValue) {
            if !UserDefaults.standard.showCasesByUserID.contains(AppConstants.Showcase.multipleCollects.rawValue) {
                UserDefaults.standard.showCasesByUserID.append(AppConstants.Showcase.multipleCollects.rawValue)
            }
            return
        }
    ...
    func showSecondBalloon(view: UIView, arrowPointsTo: UIView) {
        if UserDefaults.standard.viewedCoachMarks != 1 || UserDefaults.standard.showCasesByUserID.contains(AppConstants.Showcase.deleteMultipleCollects.rawValue){
            if !UserDefaults.standard.showCasesByUserID.contains(AppConstants.Showcase.deleteMultipleCollects.rawValue) {
                UserDefaults.standard.showCasesByUserID.append(AppConstants.Showcase.deleteMultipleCollects.rawValue)
            }
            return
        }
